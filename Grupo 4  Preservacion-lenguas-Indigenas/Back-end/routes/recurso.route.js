...
router.post('/registrar-recurso', async (req, res) => {
    try {
        let nuevoRecurso = new Recurso({
            ...
            tipoRecurso: req.body.tipoRecurso,        // audio, video, texto
...
// http://localhost:3000/api/obtener-recurso/:id
router.get('/obtener-recurso/:id', async (req, res) => {
    try {
...
// http://localhost:3000/api/editar-recurso/:id
router.put('/editar-recurso/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const datos = {
            titulo: req.body.titulo,
        ...
        await Recurso.findByIdAndUpdate(id, datos, { new: true });
